...
Provides automatic bidirectional synchronization between window focus and browser URL.

- When a window is focused → URL updates to match
- When URL changes (browser back/forward) → Corresponding window is focused/opened
...
function DesktopWithRouting() {
  // That's it! Bidirectional sync happens automatically
  useWindowRouting({
    windows,
...
| Option         | Type                           | Default           | Description                                                    |
| -------------- | ------------------------------ | ----------------- | -------------------------------------------------------------- |
| `windows`      | `WindowDefinitions`            | -                 | Window definitions from `defineWindows()` (required)           |
| `adapter`      | `RoutingAdapter`               | `browserAdapter`  | Routing adapter for URL navigation                             |
| `onPathChange` | `(componentId: string) => void`| -                 | Optional custom handler for URL→window sync (overrides default)|

### Return Value

| Property            | Type                    | Description                              |
| ------------------- | ----------------------- | ---------------------------------------- |
| `getInitialWindowId`| `() => string \| undefined` | Get componentId for current URL path |
| `getCurrentPath`    | `() => string`          | Get the current URL path                 |
...
```tsx
// For apps deployed at a subpath (e.g., /examples/astro/)
