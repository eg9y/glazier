...
import { iconConfigs, windowConfigs } from "../lib/windowConfigs";
 ...
 * Handles focus ↔ URL synchronization.
 */
export function DesktopShell({ initialWindowId }: DesktopShellProps) {
	...
	const handleFocusChange = useCallback((windowId: string | null) => {
		if (windowId) {
			// Update URL when focus changes (shallow to avoid remount)
			const path = windowId === "home" ? "/" : `/${windowId}`;
		...
		>
			...
			<Desktop className="pointer-events-none absolute inset-0">
				{({ windowId, component: Component, componentProps }) => {
					...
					const WindowContent = Component as React.ComponentType<{
						windowId: string;
